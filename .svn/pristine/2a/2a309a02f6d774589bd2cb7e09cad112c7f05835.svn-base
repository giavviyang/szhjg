<template>
  <div class="picedit">
    ...
    <!--    <div class="picright">-->
    <!--      <OpenVc style="height: 90%;width: 100%;border: 1px solid"></OpenVc>-->
    <!--    </div>-->
    <div class="picright">
      <div class="urlArr">
        <div v-for="(urls, index) in urlArr" class="urlItem" :key="`c_${index}`" @click="() => showImagesByComponent(urls)">
          <div class="imgUrl">
            <img :src="urls.url" :alt="urls.imgDes"/>
          </div>
          <div class="imgInfo">{{ urls.imgDes }}</div>
        </div>
      </div>
      <CutterImg ref="cutterImg" :currentImg="currentImg" :downloadImgSrc="downloadImgSrc" :fileName="fileName" :fileType="fileType"
                 @cutDown="cutDown" @onPrintImg="onPrintImg" @previousPage="handlePreviousPage" @nextPage="handleNextPage"
                 @imgMoveUp="imgMoveUp" @imgMoveDown="imgMoveDown" @chooseImg="chooseImg" @onChooseImg="onChooseImg"
                 @importImgToCanv="importImgToCanv" @handleNextPiece="handleNextPiece"/>
    </div>
    <!--领取任务-->
...
import CutterImg from "@/views/components/cutterImg/index.vue";
import {queryFileImageList, pictureMoveUpAndDown, queryNextPiece, imageReplacement} from "@/api/szhjg/sjjg/txcl";
...
export default {
  components: { CutterImg, ViewingReceive, ViewingRecord},
  data() {
    return {
  ...
  methods: {
    ...
    // 图像处理，打开弹窗
    showImagesByComponent(val) {
      this.$refs.cutterImg.visible = true;
      this.$refs.cutterImg.keyDown();
      this.$refs.cutterImg.toolBoxControlBoxFlag = false;
      this.currentImg = {
      ...
      this.downloadImgSrc = val.url;
    },
    // 图像处理，预览
    onPrintImg(res) {
      this.downloadImgSrc = res.dataURL;
    ...
    cutDown(res) {
      ...
      this.fileName = res.fileName;
      //保存图片
      if (res.fileName ==="2000121200000000000000000.png"){
        this.$message({message: '当前图片为提示图片，禁止提交该图片！', type: 'warning'});
        return false;
      }else {

        const file = dataURLtoBlob(res.dataURL);   //将dataURL转换为Blob对象
        // const file = res.dataURL;   //将dataURL转换为Blob对象
        const id = res.id;
        const formData = new FormData();
        formData.append('replaceFile', file, res.fileName);   //添加Blob对象及文件名
    ...
    // 图像处理，上一页
    handlePreviousPage(val) {
      if(val.pageCount>1){
        //根据传入的当前页码获取上一页页码
        const previousPage = val.pageCount - 1;
        //遍历urlArr数组，查找上一页对应的previousPage对象
        const previousImage = this.urlArr.find(image => image.pageCount === previousPage);
        //previousImage对象有值，则将图像显示并保存值；否则不进行切换
        if (previousImage) {
          this.currentImg = {
            fileCount: previousImage.fileCount,
            pageCount: previousImage.pageCount,
            name: previousImage.fileName,
            id:previousImage.id,
            src: previousImage.url,
          };
          this.fileName=previousImage.fileName;
          this.$refs.cutterImg.handleOpen(this.currentImg);
        }else {
          this.$message({message: '找不到上一页对应的图像！', type: 'warning'});
          return false;
        }
      }else {
        this.$message({message: '当前图像为首页，无法获取上一页！', type: 'warning'});
        return false;
      }
    },
    // 图像处理，下一页
    handleNextPage(val) {
      if (val.pageCount === this.urlArr.length) {
        this.$message({message: '当前图像为尾页，无法获取下一页！', type: 'warning'});
        return;
      }
      //根据传入的当前页码获取下一页页码
      const nextDown = val.pageCount + 1;
      //遍历urlArr数组，找到下一页对应的nextDown对象
      const nextImage = this.urlArr.find((image) => image.pageCount === nextDown);
      //nextImage对象有值，则将图像显示并保存值；否则不进行切换
      if (nextImage) {
        this.currentImg = {
          fileCount: nextImage.fileCount,
          pageCount: nextImage.pageCount,
          name: nextImage.fileName,
          id:nextImage.id,
          src: nextImage.url,
        };
        this.fileName=nextImage.fileName;
      }else {
        this.$message({message: '找不到下一页对应的图像！', type: 'warning'});
        return false;
      }
    },
    // 图像处理，上移
    imgMoveUp(val) {
      if(val.pageCount>1){
        //当前页的ID
        const currentPageId = val.id;
        //获取上移页的ID
        const previousPageCount = val.pageCount - 1;
        const previousImage = this.urlArr.find(image => image.pageCount === previousPageCount);
        const previousPageId = previousImage ? previousImage.id : null;
        //判断是否为空
        if (currentPageId && previousPageId) {
          this.$confirm('此操作将上移选中的图片数据, 是否继续?', '提示', {
            ...
            const _this = this;
            const formData = new FormData();
            formData.append("currentId", currentPageId);
            formData.append("editId", previousPageId);
            //保存方法
            pictureMoveUpAndDown(formData).then(res => {
              if (res.code === 200){
                _this.urlArr = [];
                _this.$message({message: "图片上移成功", type: 'success',});
                _this.editPicture(previousPageId);
              }
            });
          }).catch(() => {});
        }
      }else {
        this.$message({message: '当前图像为第一页，不能上移！', type: 'warning'});
        return false;
      }
    },
    // 图像处理，下移
    imgMoveDown(val) {
      if (val.pageCount === this.urlArr.length) {
        this.$message({message: '当前图像为最后一页，不能下移！', type: 'warning'});
        return false;
      }
      //当前页的ID
      const currentPageId = val.id;
      ...
      if (currentPageId && previousPageId) {
        this.$confirm('此操作将下移选中的图片数据, 是否继续?', '提示', {
          ...
          pictureMoveUpAndDown(formData).then(res => {
            if (res.code === 200){
              _this.urlArr = [];
              _this.$message({message: "图片下移成功", type: 'success'});
              _this.editPicture(previousPageId);
            }
          });
        }).catch(() => {});
    ...
    // 下一件
    handleNextPiece(val) {
      this.$confirm('是否切换到下一件, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
        const _this = this;
        queryNextPiece({id: val.id}).then((res) => {
          if (res.msg === "NoData"){
            _this.$message({message: '未查询到下一件信息，当前为最后一件！', type: 'warning'});
            return false;
          }else {
            _this.urlArr = [];
            const store = res.data;
            _this.urlArr = store.map(item => {item.url = item.imgUrl;delete item.imgUrl;return item;});
            _this.urlArr.map(item => {
              if (item.url) { // 判断item.url是否为空
            ...
            //获取第一条数据
            const filteredData = _this.urlArr.find(item => item.pageCount === 1);
            _this.currentImg = {
              fileCount: filteredData.fileCount,
              pageCount: filteredData.pageCount,
              name: filteredData.fileName,
              id:filteredData.id,
              src: filteredData.url,
            };
            _this.fileName=filteredData.fileName;
            //修改文件列表勾选框数据
            _this.$nextTick(function () {
              const table = _this.$refs.subdivisionTable;
              table.clearSelection();     //清除表格中的所有勾选状态
              const row = _this.subdivisionList.find(item => item.id === filteredData.id);
              if (row) {
                const index = _this.subdivisionList.indexOf(row);
                table.toggleRowSelection(_this.subdivisionList[index], true);
              }
            });
          }
        })
    ...
    subHandleSelectionChange(val) {
      ...
      if (this.subGridCheck.length == 1) {
        this.getOneData = this.subGridCheck[0];
        this.getImg(this.subGridCheck[0]);
      }
    },
    //行点击事件
    ...
    getImg(val) {
      if (val) {
        queryFileImageList({ajId: val.ajId, jh: val.jh}).then((res) => {
          const store = res.data;
          //返回修改后的对象
          this.urlArr = store.map(item => {item.url = item.imgUrl;delete item.imgUrl;return item;});
          ...
          //判断查询的是否是首页,不是首页继续执行
          if (!val.sfsy || val.sfsy !== '是') {
            const previousImage = this.urlArr.find(image => image.id === val.id);
            this.currentImg = {
              fileCount: previousImage.fileCount,
              pageCount: previousImage.pageCount,
              name: previousImage.fileName,
    ...
    //上(下)移之后重新进行查询信息
    editPicture(val){
      queryFileImageList({ajId: this.getOneData.ajId, jh: this.getOneData.jh}).then((res) => {
        const store = res.data;
        //返回修改后的对象
        this.urlArr = store.map(item => {item.url = item.imgUrl;delete item.imgUrl;return item;});
        this.urlArr.map(item => {
        ...
        //遍历urlArr数组，查根据id判断数据
        const previousImage = this.urlArr.find(image => image.id === val);
        this.currentImg = {
          fileCount: previousImage.fileCount,
          pageCount: previousImage.pageCount,
          name: previousImage.fileName,
