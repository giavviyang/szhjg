...
public class StatisticsServiceImpl implements StatisticsCountService {
    ...
    //加工进度统计
    @Override
    public List<CountVO> processProgressStatistics(StatisticsVO statisticsVO) {
        //创建一个空集合
        List<CountVO> countList = new ArrayList<>();
        //查询项目
        LambdaQueryWrapper<TProject> projectWrapper = new LambdaQueryWrapper<>();
        if (StringUtils.isNotBlank(statisticsVO.getProjectId())){
            projectWrapper.eq(TProject::getId,statisticsVO.getProjectId());
        }
        List<TProject> projectList = tProjectService.list(projectWrapper);
        for (TProject project : projectList){
            LambdaQueryWrapper<TBatch> batchWrapper = new LambdaQueryWrapper<>();
            batchWrapper.eq(TBatch::getProjectId,project.getId()).orderByDesc(TBatch::getCreateTime);
            List<TBatch> listBatch = tBatchService.list(batchWrapper);
            for (TBatch batch : listBatch){
                //实体类
                CountVO countVO = new CountVO();
                countVO.setProjectId(project.getId());
                countVO.setProjectName(project.getProjectName());
                countVO.setBatchId(batch.getId());
                countVO.setBatchName(batch.getPcName());
                countVO.setBegindate(batch.getBegindate());
                countVO.setEnddate(batch.getEnddate());
                //查询案卷
                LambdaQueryWrapper<TJuanji> juanWrapper = new LambdaQueryWrapper<>();
                juanWrapper.eq(TJuanji::getPcId,batch.getId());
                List<TJuanji> juanList = tJuanjiService.list(juanWrapper);
                // 过滤出任务状态为已完成的个数
                long maleCount = juanList.stream().filter(juan -> juan.getRwzt().equals("已完成")).count();
                //过滤出所有的案卷id
                List<String> didList = juanList.stream().map(TJuanji::getId).collect(Collectors.toList());
                if (didList.size()>0){
                    //查询文件
                    ...
                    //查找该批次下的总案卷数
                    double numCount = (double)juanList.size();
                    //计算完成进度
                    double rate = 100.0 * maleCount / numCount;
                    BigDecimal big= new BigDecimal(rate);
                    big = big.setScale(2, BigDecimal.ROUND_HALF_UP);
                    //转成字符串
                    String rateStr = big.toPlainString() + "%";
                    countVO.setCompletedProgress(rateStr);
                    countVO.setCompletedVolume(maleCount +"");

                    countVO.setFileCount(juanList.size()+"");
                    countVO.setPieceCount(a+"");
                    countVO.setPageCount(b+"");
                    countList.add(countVO);
                }else {
                    countVO.setFileCount("0");
                    countVO.setPieceCount("0");
                    countVO.setPageCount("0");
                    countVO.setCompletedVolume("0");
                    countVO.setCompletedProgress("0.00%");
                    countList.add(countVO);
                    continue;
                }
            }
