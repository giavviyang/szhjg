...
public class MetadataMapperServiceImpl implements MetadataMapperService {
    ...
    //字段管理--查询编号字段是否存在
    @Override
    public CommonResult queryNumberOne(String daId, String points) {
        //创建存放任务信息map
        ...
        map.put("msg1",tMapper.getId());

        //查询是否存在批量添加
        LambdaQueryWrapper<TMapper> bulkWrapper = new LambdaQueryWrapper<>();
        bulkWrapper.eq(TMapper::getDaId,daId).eq(TMapper::getPoints,points).eq(TMapper::getBulkAdd,"是");
        List<TMapper> list1 = mapperService.list(bulkWrapper);
        TMapper tMapper1 = new TMapper();
        if ( list1.size()>0 ){
            tMapper1 = list1.get(0);
        }
        map.put("msg2",tMapper1.getBulkAdd());
        map.put("msg3",tMapper1.getId());
    ...
    public CommonResult addNumberingField(TmetadataVO tmetadataVO) {
        ...
        if ( list.size()>0 ){
            ...
            if (arrayList.size()>0){
                ...
                boolean p = setBatchAdded(tmetadataVO);
                if ( o && p ){
    ...
    //设置批量添加字段
    private boolean setBatchAdded(TmetadataVO tmetadataVO) {
        if ("0".equals(tmetadataVO.getPoints())){
            if ("是".equals(tmetadataVO.getAppendPermit())){
                LambdaQueryWrapper<TMapper> wrapper = new LambdaQueryWrapper<>();
                wrapper.eq(TMapper::getDaId,tmetadataVO.getDaId()).eq(TMapper::getPoints,"0");
                wrapper.eq(TMapper::getBulkAdd,"是");
                List<TMapper> list = mapperService.list(wrapper);
                if ( list.size()>0){
                    list.forEach(e->{
                        e.setBulkAdd("");
                ...
                wrapper1.eq(TMapper::getId,tmetadataVO.getAppendField());
                List<TMapper> list1 = mapperService.list(wrapper1);
                if ( list1.size()>0 ){
                    TMapper mapper = list1.get(0);
                    mapper.setBulkAdd("是");
                    mapper.setBulkField("是");
                    //boolean b = mapperService.updateById(mapper);
                    return mapperService.updateById(mapper);
                }
                return true;
            }else if ("否".equals(tmetadataVO.getAppendPermit())){
                LambdaQueryWrapper<TMapper> wrapper = new LambdaQueryWrapper<>();
                wrapper.eq(TMapper::getDaId,tmetadataVO.getDaId()).eq(TMapper::getPoints,"0");
                wrapper.eq(TMapper::getBulkAdd,"是");
                List<TMapper> list = mapperService.list(wrapper);
                if ( list.size()>0){
                    ...
                    return mapperService.updateBatchById(list);
                }
                return true;
            }
        }
