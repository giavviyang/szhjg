            ...
            reader.onload = () => {
              let timmer = setInterval(() => {
                if (reader.readyState == 2) {
                  ...
                  ctx.fillStyle = "#fff";
                  ctx.fillRect(0, 0, newCanv.width, newCanv.height);
                  // ctx.clearRect(0, 0,newCanv.width, newCanv.height);
                  // ctx.fillRect(0, 0, c.width, c.height);
                  // 原图裁剪 originalGraph
                  ...
                  newCanv.toBlob(
                    (newBlob) => {
                      ...
                      if (!doNotReset) {
                        // _this.handleClose();
                        console.log('this.fileType',`image/${_this.fileType}`)
                        _this.$emit('cutDown', {
                          id: this.currentImg.id,
                          index: _this.index,
                          fileName,
                        ...
                        if (_this.previewMode) {
                          console.log('111this.fileType', `image/${_this.fileType}`,newCanv.toDataURL(
                            `image/${_this.fileType}`,
                            _this.quality
                          ),)
