      ...
      <div class="dashboard-editor-container">
        ...
        <div style="height: 35%">
          <div class="chart-wrapper" style="white-space: nowrap; overflow-x: auto; overflow-y: hidden;">
            <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-hasPermi="['szhjg:sjjg:txzjhcl:search']">
          ...
          <div class="chart-wrapper" style="white-space: nowrap; overflow-x: auto; overflow-y: hidden;">
            <el-form :model="queryForm" ref="queryForm" size="small" :inline="true"
              ...
              <el-form-item label="编号" prop="bh">
                <el-input v-model="queryForm.bh" :disabled="true" clearable class="rt-input" style="width: 200px"
                          size="mini" />
              </el-form-item>
...
import { queryZjHClTaskList } from "@/api/szhjg/sjjg/txzj";
...
export default {
  ...
  methods: {
    ...
    //默认查询项目批次
    getOptionsData() {
      queryProjectBatch({gxId: 6}).then((res) => {
    ...
    queryTaskNameList(params) {
      const _this = this
      queryZjHClTaskList(params, {emulateJSON: true}).then(function (response) {
    ...
    submitTask(){
      ...
      if (this.taskGridCheck.length == 1){
        ...
        queryNextGxName({id:a, gxName:6}).then(response =>{
          var o;
          if (response.msg =="图像质检处理"){
            if(this.taskGridCheck[0].cataloged =="著录完成"){
              o = "当前工序 :图像质检处理。已是最后一道工序,是否提交!"
            }else {
              this.$message({message: '当前工序 :图像质检处理。已是最后一道工序，未进行目录著录，无法提交完成！', duration: 3000, type: 'error'});
              return;
            }
          }else {
            o = "当前工序：图像质检处理，下一道工序为："+response.msg+"。是否提交!"
          }
          this.$confirm(o, '提示', {
            confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning',
          }).then(() => {
            processingSubmitPass({ajId:a, gxId:6}).then(response =>{
