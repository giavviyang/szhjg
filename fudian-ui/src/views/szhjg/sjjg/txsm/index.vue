<template>
  <div class="picedit">
      ...
      <div class="dashboard-editor-container">
        ...
        <div style="height: 35%">
          <div class="chart-wrapper" style="white-space: nowrap; overflow-x: auto; overflow-y: hidden;">
            <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-hasPermi="['szhjg:sjjg:txsm:search']">
    ...
    <div class="picright">
      <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick" style="height: 100%;margin-top: 3px">

        <el-tab-pane label="扫描" name="second" >
          <div>
            <!-- 这里是右半部分的内容，使用动态组件来渲染另一个Vue组件 -->
            <component :is="youchuScan"></component>
          </div>
        </el-tab-pane>

        <el-tab-pane label="浏览图像" name="first">
...
import { queryTxsmTaskList } from "@/api/szhjg/sjjg/txsm";
...
import { uploadEditImg } from "@/api/szhjg/sjjg/txcl";
import YouchuScan from './YouchuScan.vue';
...
export default {
  ...
  data() {
    return {
      ...
      /*------------------------------        扫描/查看图像的参数     ----------------------------------------------------*/
      youchuScan: 'YouchuScan', // 这里指定要渲染的组件的名称
      activeName: 'second',
  ...
  methods: {
    ...
    //默认查询项目批次
    getOptionsData() {
      queryProjectBatch({gxId: 3}).then((res) => {
    ...
    queryTaskNameList(params) {
      const _this = this
      queryTxsmTaskList(params, {emulateJSON: true}).then(function (response) {
    ...
    submitTask(){
      ...
      if (this.taskGridCheck.length == 1){
        ...
        queryNextGxName({id:a, gxName:b}).then(response =>{
          var o;
          if (response.msg =="图像扫描"){
            if(this.taskGridCheck[0].cataloged =="著录完成"){
              o = "当前工序 :图像扫描。已是最后一道工序,是否提交!"
            }else {
              this.$message({message: '当前工序 :图像扫描。已是最后一道工序，未进行目录著录，无法提交完成！', duration: 3000, type: 'error'});
              return;
            }
          }else {
            o = "当前工序：图像扫描，下一道工序为："+response.msg+"。是否提交!"
          }
          this.$confirm(o, '提示', {
