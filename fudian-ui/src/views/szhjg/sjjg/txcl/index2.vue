<template>
  <div class="picedit">
    ...
    <div class="picright">
      <div class="urlArr">
        <div v-for="(urls, index) in urlArr" class="urlItem" :key="`c_${index}`" @click="() => showImagesByComponent()">
          <div class="imgUrl">
            <img :src="urls"/>
          </div>
          <div class="imgInfo">第{{ index + 1 }}页</div>
        </div>
      </div>
      <CutterImg ref="cutterImg" :currentImg="currentImg" :downloadImgSrc="downloadImgSrc" :fileName="fileName"
                 @cutDown="cutDown" @onPrintImg="onPrintImg" @previousPage="handlePreviousPage"
                 @nextPage="handleNextPage" @imgMoveUp="imgMoveUp"
                 @imgMoveDown="imgMoveDown" @chooseImg="chooseImg" @onChooseImg="onChooseImg" @importImgToCanv="importImgToCanv"/>
    </div>
    <!--领取任务-->
...
export default {
  ...
  methods: {
    // 上一页
    previousPage(val) {
      val = 'https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c1d7a1feb60346449c1a64893888989a~tplv-k3u1fbpfcp-watermark.image';
      this.url = val;
      // this.init();
    ...
    showImagesByComponent() {
      ...
      this.$refs.cutterImg.currentPageCount=1;
      this.$refs.cutterImg.currentFileCount=1;
      this.currentImg = {
        fileCount:1,
        pageCount:2,
        name: '111',
    ...
    handleNextPage(val) {
      ...
      this.currentImg = {
        ...
        pageCount:  val.pageCount+1,
        name: '111',
        src: 'https://pic3.zhimg.com/v2-c6ae9c3aff36b9b221258f6a90577902_r.jpg',
      };
    },
    // 图像处理，上移
    imgMoveUp(val) {
    ...
    getImg(val) {
      if (val) {
        ...
        this.urlArr = [
          {
            url: '/dev-api/szhjgCommonController/ioReadImg2?url=D%3A%5Cmyprojects-szhjgxt%5Cimage2%5C7-1-1.png',
            des: '第1页'
          }, {
            url: '/dev-api/szhjgCommonController/ioReadImg2?url=D%3A%5Cmyprojects-szhjgxt%5Cimage2%5C7-2-1.jpg',
            des: '第2页'
          }, {
            url: '/dev-api/szhjgCommonController/ioReadImg2?url=D%3A%5Cmyprojects-szhjgxt%5Cimage2%5C7-3-2.jpg',
            des: '第3页'
          }, {
            url: '/dev-api/szhjgCommonController/ioReadImg2?url=D%3A%5Cmyprojects-szhjgxt%5Cimage2%5C7-2-1.jpg',
          }, {
