...
public class BatchManagementController {
    ...
    @ApiOperation("导出完成数据包")
    @ApiOperationSupport(order = 41)
    @PostMapping("/exportCompletedInformation")
    public CommonResult<?> exportCompletedInformation(TBatch batch){
        return batchManagementService.exportCompletedInformation(batch);
    }
    ...
    public CommonResult<?> getzipInformation(@PathVariable("id") String id) {
        return batchManagementService.getzipInformation(id);
    }

    @RequestMapping(value = "/uploader/downloadBigFile", method = RequestMethod.GET)
    public void downloadBigFile(HttpServletRequest request, HttpServletResponse response) throws IOException {
